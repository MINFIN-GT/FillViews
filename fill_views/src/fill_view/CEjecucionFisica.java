...
public class CEjecucionFisica {
	...
	public static boolean loadEjeucionFisica(Connection conn, Integer ejercicio){
		boolean ret = true;
		try{
			...
			if(!conn.isClosed()){
				pstm = conn.prepareStatement("SELECT * FROM dashboard.mv_ejecucion_fisica where ejercicio = ? ");
				pstm.setInt(1, ejercicio);
				...
				if(rs!=null && bconn){
					...
					while(rs.next()){
						if(first){
							pstm2 = CMemSQL.getConnection().prepareStatement("delete from mv_ejecucion_fisica where ejercicio = ?")  ;
				...
				pstm = conn.prepareStatement("SELECT * FROM sicoinprod.sf_meta where ejercicio = ? ");
				pstm.setInt(1, ejercicio);
				...
				if(rs!=null){
					...
					while(rs.next()){
					    ...
					    Timestamp fecha_inicio = rs.getString("fecha_inicio")!=null ? new Timestamp(dateFormat.parse(rs.getString("fecha_inicio")).getTime()) : null;
					    Timestamp fecha_fin = rs.getString("fecha_fin")!=null ? new Timestamp(dateFormat.parse(rs.getString("fecha_fin")).getTime()) : null;
	...
	public static boolean loadEjeucionFisicaHistoria(Connection conn, Integer ejercicio_inicio, Integer ejercicio_fin){
		boolean ret = true;
		try{
			...
			if(!conn.isClosed()){
				...
				if(rs!=null && bconn){
					...
					while(rs.next()){
						if(first){
							pstm2 = CMemSQL.getConnection().prepareStatement("delete from mv_ejecucion_fisica where ejercicio between ? and ?")  ;
			...
			CLogger.writeFullConsole("Error 4: CEjecucionFisica.class", e);
		}
		finally{
			CMemSQL.close();
		}
